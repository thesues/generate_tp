...
static void vfs_lttng_init_search_op(vfs_handle_struct *handle,
					  DIR *dirp)

{
        tracepoint(vfs_lttng, vfs_lttng_init_search_op_enter );
        SMB_VFS_NEXT_INIT_SEARCH_OP(handle, dirp);
        tracepoint(vfs_lttng, vfs_lttng_init_search_op_exit);
}
...
static ssize_t vfs_lttng_read(vfs_handle_struct *handle,
				   files_struct *fsp, void *data, size_t n)

{
        ssize_t result;
        tracepoint(vfs_lttng, vfs_lttng_read_enter , fsp, n);
        result = SMB_VFS_NEXT_READ(handle, fsp, data, n);
        tracepoint(vfs_lttng, vfs_lttng_read_exit, result);
        return result;
}
...
static ssize_t vfs_lttng_write(vfs_handle_struct *handle,
				    files_struct *fsp,
				    const void *data, size_t n)

{
        ssize_t result;
        tracepoint(vfs_lttng, vfs_lttng_write_enter , fsp, n);
        result = SMB_VFS_NEXT_WRITE(handle, fsp, data, n);
        tracepoint(vfs_lttng, vfs_lttng_write_exit, result);
        return result;
}
...
{
        uint64_t result;
        tracepoint(vfs_lttng, vfs_lttng_get_alloc_size_enter , fsp);
...
static int vfs_lttng_chmod_acl(vfs_handle_struct *handle,
			const struct smb_filename *smb_fname,
			mode_t mode)

{
        int result;
        tracepoint(vfs_lttng, vfs_lttng_chmod_acl_enter , smb_fname, mode);
        result = SMB_VFS_NEXT_CHMOD_ACL(handle, smb_fname, mode);
        tracepoint(vfs_lttng, vfs_lttng_chmod_acl_exit, result);
        return result;
}


static int vfs_lttng_fchmod_acl(vfs_handle_struct *handle,
				     files_struct *fsp, mode_t mode)

{
        int result;
        tracepoint(vfs_lttng, vfs_lttng_fchmod_acl_enter , fsp, mode);
        result = SMB_VFS_NEXT_FCHMOD_ACL(handle, fsp, mode);
        tracepoint(vfs_lttng, vfs_lttng_fchmod_acl_exit, result);
        return result;
}
...
static struct vfs_fn_pointers vfs_lttng_fns = {
        ...
        .chmod_acl_fn = vfs_lttng_chmod_acl,
        .fchmod_acl_fn = vfs_lttng_fchmod_acl,
